 ...
 * 2. Admin API key pool (active real keys in admin pool with auto-rotation)
 * 3. System environment variable (process.env.GEMINI_API_KEY) as ultimate fallback
 */
export function resolveApiKey(customApiKeyInput?: string, clientAccessCode?: string): ResolvedKeyContext {
  ...
  // 2. Admin API key pool resolution with Auto-Rotation (Least-Used & Health check)
  const now = Date.now();
  ...
  // Fallback within admin pool: try any active real key not in cooldown
  ...
  // 3. Fallback to server environment key if available when admin pool is empty
