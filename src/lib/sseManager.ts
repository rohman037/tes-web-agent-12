...
class SSEManager {
  ...
  private handleIncomingPayload(payload: any) {
    ...
    if (payload.type === 'clients_updated') {
      if (Array.isArray(payload.clients) && typeof localStorage !== 'undefined') {
        try {
          localStorage.setItem('satset_clients', JSON.stringify(payload.clients));
        } catch {}
      }
