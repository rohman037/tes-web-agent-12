...
export default function SystemHealthWidget() {
  ...
  const checkHealth = async () => {
    ...
    try {
      ...
      if (dbRes.ok && dbData.ok && (dbData.status === 'CONNECTED' || dbData.mode === 'REMOTE_FIRESTORE')) {
        ...
        // Double check client-side Firestore connection directly
        const clientOk = await testConnection();
        if (clientOk) {
          setDbStatus({ ok: true, statusText: 'Firestore Connected (Live Node)' });
        } else {
          setDbStatus({ ok: true, statusText: `Resilient Local DB (${dbData?.status || 'Active'})` });
        }
      }
    } catch (dbErr) {
      const clientOk = await testConnection().catch(() => false);
      if (clientOk) {
        setDbStatus({ ok: true, statusText: 'Firestore Connected (Live Node)' });
      } else {
        setDbStatus({ ok: true, statusText: 'Resilient Local DB (Active)' });
      }
    }
