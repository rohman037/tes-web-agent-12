...
export default function ClientMonitoringPanel() {
  ...
  const loadData = async (manual = false) => {
    ...
    // Strict safety timer: Loading never hangs beyond 3.5 seconds
    ...
    try {
      const res = await syncClientsAsync();
      if (res && res.clients && Array.isArray(res.clients)) {
        setClients(res.clients);
      }
    } catch (err) {
      console.warn('[ClientMonitoringPanel] loadData error caught:', err);
    } finally {
  ...
  useEffect(() => {
    loadData();
    window.addEventListener('satset_clients_updated', refreshLocalState);
    window.addEventListener('satset_packages_updated', refreshLocalState);
    window.addEventListener('satset_history_updated', refreshLocalState);
    window.addEventListener('storage', refreshLocalState);

    // Subscribe to unified live generation & client sync events
    const unsubscribeLive = subscribeLiveGenerationEvents((data) => {
      // 1. Live generation tracker
      ...
      // 2. Real-time client updates from SSE
      if ((data.type === 'clients_updated' || data.type === 'snapshot') && Array.isArray(data.clients)) {
        setClients(data.clients);
        setIsLoading(false);
      }
    });

    // Also attach Firestore real-time listener if available
    const unsubscribeFirestore = subscribeToClients();

    return () => {
      window.removeEventListener('satset_clients_updated', refreshLocalState);
      window.removeEventListener('satset_packages_updated', refreshLocalState);
      window.removeEventListener('satset_history_updated', refreshLocalState);
  ...
  const handleAddClientSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!addName.trim()) {
      showToast('Mohon isi nama client.');
      return;
    }
    ...
    showToast(`✅ Klien "${newClient.name}" berhasil ditambahkan! Kode: ${newClient.accessCode}`);
  };
  ...
  const handleToggleSuspend = (client: ClientItem) => {
    ...
    refreshLocalState();
    showToast(`Status klien "${client.name}" diubah menjadi: ${newStatus.toUpperCase()}`);
  };

  const handleConfirmExtend = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClientForExtend) return;
    const clientName = selectedClientForExtend.name;
    const days = Number(extendDaysInput) || 7;
    extendClientExpiry(selectedClientForExtend.id, days);
    setSelectedClientForExtend(null);
    refreshLocalState();
    showToast(`Masa aktif ${clientName} berhasil diperpanjang +${days} hari!`);
  };
  ...
  const handleConfirmPackageChange = (e: React.FormEvent) => {
    ...
    if (targetPkg) {
      const clientName = selectedClientForPackage.name;
      updateClientPackage(
      ...
      refreshLocalState();
      showToast(`Paket ${clientName} berhasil diubah menjadi "${targetPkg.name}"!`);
    }
  };
  ...
  const handleConfirmDelete = () => {
    ...
    refreshLocalState();
    showToast(`Klien "${name}" berhasil dihapus.`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {toastMessage && (
        <div className="p-3.5 rounded-xl bg-indigo-50 border border-indigo-200 text-xs font-bold text-[#3525cd] flex items-center justify-between shadow-xs animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center gap-2">
            <Info className="w-4 h-4 text-[#3525cd] shrink-0" />
            <span>{toastMessage}</span>
          </div>
          <button
            type="button"
            onClick={() => setToastMessage(null)}
            className="p-1 text-indigo-400 hover:text-indigo-600 rounded-lg cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard
          ...
          isLoading={isLoading && safeClients.length === 0}
        />
        ...
        headerActions={
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 text-xs font-bold transition-all flex items-center gap-1.5 border border-slate-200 cursor-pointer disabled:opacity-60 shadow-2xs"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? 'animate-spin text-[#3525cd]' : ''}`} />
              <span>{isRefreshing ? 'Sinkronisasi...' : 'Refresh'}</span>
            </button>
            <button
              type="button"
              onClick={() => setShowAddClientModal(true)}
              className="px-3.5 py-2 rounded-xl bg-[#3525cd] hover:bg-[#2c1eb3] text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-xs cursor-pointer"
            >
